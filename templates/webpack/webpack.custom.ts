...
export default (config: Configuration, angularJsonConfig: CustomWebpackBrowserSchema, targetOptions: TargetOptions) => {
  ...
  traverse(
    config,
    v => typeof v === 'string' && v.includes('src/styles/themes/default'),
    (obj, key) => {
      obj[key] = (obj[key] as string).replace('src/styles/themes/default', 'src/styles/themes/' + firstConfiguration);
    }
  );
